...
class Game:
    ...
    #     query = "SELECT * FROM users JOIN games ON users.id = games.user_id WHERE users.id = games.user_id;"
    #     games_from_db = connectToMySQL(cls.db).query_db(query)
    #     all = []
    #     print("JOIN QUERY", games_from_db)
    #     #games_instance = cls(games_from_db[0])
    #     if not games_from_db:
    #         print("NO RESULTS FROM JOIN QUERY")
    #         return False
    #         # print("USER_INSTANCE:", games_instance)
    ...
    #             'created_at' : game['games.created_at'],
    #             'updated_at' : game['games.updated_at']
    #         }

    #         game_inst = cls(game_data)
    #         game_inst.users = usr.User(user_data)
